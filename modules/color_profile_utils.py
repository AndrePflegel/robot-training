...
def detect_boundary_color(hsv, frame_width, frame_height, color_config):
    ...
    roi_start_y = int(frame_height * 0.40)
    ...
    absolute_y = y + roi_start_y
    bottom_y = absolute_y + h

    is_wide_line = w > frame_width * 0.25
    is_tall_line = h > (frame_height - roi_start_y) * 0.20
    ...
    if not found:
        return {
            ...
            "area": area,
            "roi_start_y": roi_start_y,
            "bbox": (x, absolute_y, w, h),
            "debug_mask": roi
        }

    if bottom_y >= int(frame_height * 0.88):
        zone = "danger"
        danger = True
    elif bottom_y >= int(frame_height * 0.70):
        zone = "warning"
        danger = False
    else:
        zone = "safe"
